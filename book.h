...
namespace book
{
    ...
    {
    private:
        std::queue<order::Order> queue;
        mutable std::mutex mutex;
        std::condition_variable cv;

    public:
        void push(order::Order order)
        {
            {
        ...
        std::optional<order::Order> pop()
        {
            std::unique_lock lock(mutex);
            cv.wait(lock, [this]
                    { return !queue.empty(); });

            order::Order order = std::move(queue.front());
            queue.pop();
            return order;
        }
    };
    ...
    {
    private:
        std::map<double, std::deque<order::LimitOrder>> buyOrders;
        std::map<double, std::deque<order::LimitOrder>> sellOrders;
        mutable std::mutex mutex;
        ...
        */
        template <order::ValidOrderType T>
        void executeOrder(T &order, std::map<double, std::deque<order::LimitOrder>> &bookSide, bool timeDependent = false)
        {
            while (order.quantity > 0 && !bookSide.empty())
            {
                auto bestPriceIt = (order.side == order::Side::BUY) ? bookSide.begin() : std::prev(bookSide.end());
        ...
        */
        void handleOrder(order::LimitOrder &&order)
        {
            addOrder(std::move(order));
            matchOrders();
        }

        void handleOrder(order::MarketOrder &&order)
        {
        ...
        */
        void addOrder(order::LimitOrder const &order)
        {
            {
                auto &bookSide = (order.side == order::Side::BUY) ? buyOrders : sellOrders;
        ...
        */
        void fillOrder(order::MarketOrder &&marketOrder)
        {
            auto &bookSide = (marketOrder.side == order::Side::BUY) ? sellOrders : buyOrders;
    ...
    public:
        ...
        */
        void processOrder(order::Order &&order)
        {
            std::lock_guard lock(mutex);
