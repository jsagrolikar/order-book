...
*/
void orderProcessor(std::unique_ptr<OrderBook> &book, std::unique_ptr<OrderQueue> &queue)
{
...
*/
void producer(std::unique_ptr<OrderBook> &book, std::unique_ptr<OrderQueue> &queue, std::atomic<uint64_t> &orderId)
{
    ...
    while (orderCounter.load(std::memory_order_relaxed) < MAX_ORDERS)
    {
        ...
        {
            MarketOrder order{id, sides[bernoulliDist(gen)], qtyDist(gen)};
            queue->push(std::move(order));
        }
